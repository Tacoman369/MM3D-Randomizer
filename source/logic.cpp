...
namespace Logic {
	...
	void UpdateHelpers() {
		...
		CanRideEpona = Epona && EponasSong;
		CanPlantBean = MagicBean || MagicBeanPack;
		CanOpenStormGrotto = SongOfStorms;
		CanOpenBombGrotto = CanBlastOrSmash;
		DekuStickFighting = DekuStick;
		HasFireSource = FireArrows;
		HasFireSourceWithTorch = DekuStick;

		CanUseProjectile = HasExplosives || Bow;
